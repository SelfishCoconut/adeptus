...
2. **`react-cytoscapejs` on React 19 — fallback acceptable? — RESOLVED (no fallback needed).** `react-cytoscapejs@2.0.0` declares `peerDependencies: { react: ">=15.0.0", cytoscape: "^3.2.19" }`; React 19 satisfies that open range. Installed `cytoscape@3.34.0` + `react-cytoscapejs@2.0.0` via pnpm with **no peer-dep warning and no `overrides`** needed, and the types type-check against the project tsconfig (cytoscape ships its own `index.d.ts`; `@types/react-cytoscapejs` supplies the wrapper's). The named library is used directly — no thin-core-wrapper fallback, no ADR-worthy deviation. Original concern (lib pins React ≤18) did not hold for v2.0.0.
